...
export function TechBadge({ name, active = false, onClick }: TechBadgeProps) {
  const baseClasses =
    "font-[family-name:var(--font-mono)] text-xs px-3 py-1.5 rounded-sm transition-all whitespace-nowrap border flex items-center justify-center h-[30px] box-border";
  ...
  return (
    <span className={`${baseClasses} cursor-default bg-surface/80 text-kiroshi-cyan border-kiroshi-cyan/30 hover:border-kiroshi-cyan/60 hover:shadow-[0_0_8px_rgba(85,234,212,0.2)] transition-all`}>
      {name}
    </span>
  );
}
