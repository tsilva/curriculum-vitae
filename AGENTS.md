...
2. **`scripts/generate-cv.ts`** — Parses frontmatter .md + misc.yaml → `CV.md`
...
3. Run `cd web && npm run assemble && npm run generate:cv` to regenerate outputs
4. **Do NOT hand-edit CV.md** — it is generated and will be overwritten
...
3. Run `cd web && npm run assemble && npm run generate:cv` to update outputs
...
npm run build            # Assemble + generate CV.md + build static export
npm run assemble         # Only regenerate cv-data.json from data/
npm run generate:cv      # Only regenerate CV.md from data/
```
...
3. `scripts/generate-cv.ts` reads the same data → `CV.md`
...
### CV.md is Generated
CV.md is generated by `scripts/generate-cv.ts` from `data/`. Do not hand-edit it. Edit the corresponding `data/**/*.md` or `data/misc.yaml` files instead.
...
2. Run `cd web && npm run assemble && npm run generate:cv`
